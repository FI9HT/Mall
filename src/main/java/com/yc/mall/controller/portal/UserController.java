...
/**
 * 在Controller层，只需要进行请求的应答，具体应答的内容全部交给Service层来解决，比如验证username是否存在，获取内容等等
 * Service层调用的是Dao层的接口，Dao层中声明的方法都会在*Mapper.xml文件中实现具体的Sql语句
 */
@Controller
@RequestMapping("/user")
public class UserController {
    ...
    public ServerResponse<User> getUserInfo(HttpSession session){
        //这里好像不用强制转换的啊，下面用的参数本来就是String类型的
    ...
    public ServerResponse<User> get_information(HttpSession session){
        User currentUser = (User)session.getAttribute(Const.CURRENT_USER);
        if(currentUser == null){
            return ServerResponse.createByErrorCodeMessage(ResponseCode.NEED_LOGIN.getCode(), "未登录，请强制登录");
        }
